...
namespace UnityEngine.XR.Content.Interaction
{
    ...
    public class XRKnob : XRBaseInteractable
    {
        ...
        struct TrackedRotation
        {
            ...
            public void SetBaseFromVector(Vector3 direction, AXIS axis)
            {
                ...
                {
                    case AXIS.X_Axis:
                        m_BaseAngle = Mathf.Atan2(direction.y, direction.z) * Mathf.Rad2Deg;
                        break;
                    case AXIS.Y_Axis:
            ...
            public void SetTargetFromVector(Vector3 direction, AXIS axis)
            {
                // Set the target angle
                //var targetAngle = Mathf.Atan2(direction.z, direction.x) * Mathf.Rad2Deg;
                var targetAngle = axis switch
                {
                    AXIS.X_Axis => Mathf.Atan2(direction.y, direction.z) * Mathf.Rad2Deg,
                    AXIS.Y_Axis => Mathf.Atan2(direction.z, direction.x) * Mathf.Rad2Deg,
                    AXIS.Z_Axis => Mathf.Atan2(direction.x, direction.y) * Mathf.Rad2Deg,
                    _ => 0f
                };
                // Return the offset
        ...
        void UpdateRotation(bool freshCheck = false)
        {
            ...
            if (radiusOffset >= m_PositionTrackedRadius)
            {
                if (!m_PositionDriven || freshCheck)
                {
                    m_PositionAngles.SetBaseFromVector(localOffset, axis);
            ...
            if (localY > 0.707f)
            {
                ...
                {
                    m_UpVectorAngles.SetBaseFromVector(localUp, axis);
            ...
            else
            {
                if (m_UpVectorDriven || freshCheck)
                {
                    m_ForwardVectorAngles.SetBaseFromVector(localForward, axis);
            ...
            if (m_PositionDriven)
                m_PositionAngles.SetTargetFromVector(localOffset, axis);

            if (m_UpVectorDriven)
                m_UpVectorAngles.SetTargetFromVector(localUp, axis);
            else
                m_ForwardVectorAngles.SetTargetFromVector(localForward, axis);
        ...
        void SetKnobRotation(float angle)
        {
            ...
            if (m_Handle != null)
                ...
                m_Handle.localEulerAngles = axis switch
                {
                    AXIS.X_Axis => new Vector3(angle, 0.0f, 0.0f),
                    AXIS.Y_Axis => new Vector3(0.0f, angle, 0.0f),
                    AXIS.Z_Axis => new Vector3(0.0f, 0.0f, angle),
                    _ => new Vector3(0.0f, angle, 0.0f)
                };
        }
        ...
        void OnDrawGizmosSelected()
        {
            ...
            var circleX = axis switch
            {
                AXIS.X_Axis => transform.up,
                AXIS.Y_Axis => transform.right,
                AXIS.Z_Axis => transform.right,
                _ => transform.right
            };
            var circleY = axis switch
            {
                AXIS.X_Axis => transform.forward,
                AXIS.Y_Axis => transform.forward,
                AXIS.Z_Axis => transform.up,
                _ => transform.forward
            };
